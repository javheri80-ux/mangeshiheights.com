...
function validateCaptcha(event, formobj, type) {
  ...
  const email = currentForm.find('.email-field').val();
  const comments = currentForm.find('.textarea-field').val();
  const subject = 'Mangeshi Heights Balkum';
  const source = 'mangeshi.com/mangeshi-heights.php';
  //currentForm.find('.submit_box').hide();
  //SEND EMAIL VIA AJAX 

  //var captcha = $('#captcha').val();
  var captcha = currentForm.find('#captcha').val();
  $.post("submit_demo_captcha.php?"+$("#"+form_id).serialize(), {"captcha":captcha }, function(response){
     //alert(response);
     if(response == 1){
